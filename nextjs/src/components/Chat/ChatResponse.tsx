...
const LLMResponse = ({ response, conversation, setConversations, onSaveWorkflow, existingWorkflowId }: LLMResponseProps) => {
    return (
        <>
            {
                [AI_MODAL_NAME.SONAR, AI_MODAL_NAME.SONAR_REASONING_PRO].includes(conversation?.responseModel) ? (
                    ...
                    : (
                        <div className="relative group">
                            ...
                            >
                                {MarkOutPut(response, onSaveWorkflow, existingWorkflowId)}
                            </div>
                        </div>
...
const ChatResponse = ({ conversations, i, loading, answerMessage, m, isStreamingLoading, proAgentCode, onResponseUpdate, onResponseEdited, setConversations, onSaveWorkflow }) => {
    ...
    return m?.response?.startsWith('images') ? (
        ...
        <div className="flex flex-col items-start gap-4 break-words min-h-5">
            <div
                className={`chat-content relative ${m?.responseAPI !== API_TYPE_OPTIONS.PRO_AGENT ? 'max-w-[calc(100vw-95px)] lg:max-w-none' : ''
                    ...
                    <>
                        {loading ? (
                            ...
                            isEditing ? (
                                ...
                                <LLMResponse response={answerMessage} conversation={m} setConversations={setConversations} onSaveWorkflow={onSaveWorkflow} />
                            )
                        ) : (
                            ...
                            <>
                                {isEditing ? (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="flex bg-gray-100 rounded-lg p-1 shadow-sm border border-gray-200">
                                                        <button
                                                            ...
                                                            className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${viewMode === 'plaintext'
                                                        ...
                                                        <button
                                                            ...
                                                            className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${viewMode === 'markdown'
                                                                    ? 'bg-white text-gray-900 shadow-sm border border-gray-200'
                                                                    : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                                                                }`}
                                                        >
                                    ...
                                    <LLMResponse response={m.response} conversation={m} setConversations={setConversations} onSaveWorkflow={onSaveWorkflow} />
                                )}
                                {
